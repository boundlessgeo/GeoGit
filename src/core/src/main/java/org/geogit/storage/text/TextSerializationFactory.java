...
public class TextSerializationFactory implements ObjectSerialisingFactory {
     ...
     * {@code "id" + "\t" +  <id> + "\n"}
     * {@code "name" + "\t" +  <name> + "\n"}
     * {@code "<attribute_name>" + "\t" +  <attribute_type> + "\t" + <min_occur> + "\t" + <max_occur> +  "\n"}
     * .
     * .
     * .
     * 
     * </pre>
     * 
     * Geometry attributes have an extra tokens per line representing the crs
     * 
     */
    private static final TextWriter<RevFeatureType> FEATURETYPE_WRITER = new TextWriter<RevFeatureType>() {
        ...
        protected void print(RevFeatureType featureType, Writer w) throws IOException {
            ...
            ImmutableList<PropertyDescriptor> attribs = featureType.sortedDescriptors();
        ...
        private void printAttributeDescriptor(Writer w, PropertyDescriptor attrib)
                throws IOException {
            print(w, attrib.getName().toString(), "\t", attrib.getType().toString(), "\t",
                    Integer.toString(attrib.getMinOccurs()), "\t",
                    Integer.toString(attrib.getMaxOccurs()), Boolean.toString(attrib.isNillable()));
    ...
    private static final TextReader<RevFeature> FEATURE_READER = new TextReader<RevFeature>() {
        ...
        @Override
        protected RevFeature read(BufferedReader reader) throws IOException {
            String id = parseLine(reader.readLine(), "id");
    ...
    private static final TextReader<RevFeatureType> FEATURETYPE_READER = new TextReader<RevFeatureType>() {
        ...
        @Override
        protected RevFeatureType read(BufferedReader reader) throws IOException {
            ...
            return new RevFeatureType(ObjectId.valueOf(id), type);
        ...
        private AttributeDescriptor parseAttributeDescriptor(String line) {
            ...
            if (Geometry.class.isAssignableFrom(type)) {
                ...
                attributeDescriptor = typeFactory.createGeometryDescriptor(
                        (GeometryType) attributeType, propertyName, min, max, nillable,
                        defaultValue);
            } else {
                ...
                attributeDescriptor = typeFactory.createAttributeDescriptor(attributeType,
                        propertyName, min, max, nillable, defaultValue);
            }
