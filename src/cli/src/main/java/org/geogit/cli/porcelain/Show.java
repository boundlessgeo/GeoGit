...
public class Show implements CLICommand {
    ...
    public void run(GeogitCLI cli) throws Exception {
        ...
        if (revObject instanceof RevFeature) {
            String featureTypePath = NodeRef.parentPath(path);
            RevFeatureType ft = geogit.command(ResolveFeatureType.class)
                    .setFeatureType(featureTypePath).call().get();
            ...
            ImmutableList<PropertyDescriptor> attribs = ft.sortedDescriptors();
            Ansi ansi = AnsiDecorator.newAnsi(true);
            ansi.fg(Color.YELLOW).a("ID:  ").reset().a(ft.getId().toString()).newline().newline();
            ansi.a("ATTRIBUTES  ").newline();
            ansi.a("----------  ").newline();
            ...
            ansi.a("Date:    (").fg(Color.RED)
                    .a(estimateSince(geogit.getPlatform(), commit.getTimestamp())).reset().a(") ")
